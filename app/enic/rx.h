...
void RX_PktEnqueue(LIST_HeadTypeDef *list, RX_ShadowDescTypeDef *desc);
void RX_PktDequeue(LIST_HeadTypeDef *list, RX_ShadowDescTypeDef *desc);

int32_t RX_RequestIPsecDec(int32_t gmac, int32_t qid, void *data);
void *RX_MakePCIEDmaDescriptor(int32_t gmac, int32_t qid, uint32_t dma_id, intptr_t *src, intptr_t *dst, uint32_t len);
int32_t RX_FetchDesc(int32_t gmac, int32_t qid);
int32_t RX_FillDMARing(int32_t gmac, int32_t qid, RX_ShadowDescTypeDef *sdesc);
int32_t RX_FlushData(int32_t gmac, int32_t qid, RX_ShadowDescTypeDef *msg);
...
extern RX_ShadowDescTypeDef rxdesc_shadow[NIC_QPAIR_NUM][NIC_SHADOW_RXDESC_PER_QUEUE];
#endif
