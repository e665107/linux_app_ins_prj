 ...
 * Using those would require copying too much infrastructure
 */
#define Min(x, y) ({                                                           \
        typeof(x) _min1 = (x);                                                 \
        typeof(y) _min2 = (y);                                                 \
...
#define GET_FIELD(flags, bitpos, mask)                                         \
    (((flags) >> (bitpos)) & (mask))
#define SET_FIELD(flags, val, bitpos, mask)                                    \
    (flags) = (((flags) & (~((mask) << (bitpos)))) | (((val) & (mask)) << (bitpos)))

#define GET_FLAG(flags, bitpos)                                                \
    (((flags) >> (bitpos)) & 1)
#define SET_FLAG(flags, bitpos)                                                \
    (flags) = ((flags) | (1 << (bitpos)))
#define CLEAR_FLAG(flags, bitpos)                                              \
    (flags) = ((flags) & (~(1 << (bitpos))))
